...
- Runtime liveness hint:
  - `state/comfyui.pid`
- Transfer artifact:
  - `env export` bundle directory (`manifest.json`, `pylock.toml`, `state/plugins.json`, `custom_nodes/*`, audit files)
...
- `env export` copies truth files and source snapshots into a transport bundle but does not redefine data ownership.
